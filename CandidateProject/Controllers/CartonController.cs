...
namespace CandidateProject.Controllers
{
    public class CartonController : Controller
    {
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            Carton carton1 = db.Cartons.Find(id);
            db.Cartons.Remove(carton1);
            db.SaveChanges();
            return RedirectToAction("Index");
        }
        ...
        public ActionResult AddEquipment(int? id,int? count)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            else
            {
                if(count >= 10)
                {
                    ViewBag.ErrorMsg = "More than 10 Equipments not allowed for this carton.";
                }
            }
            var carton = GetEquipmentList(id);
            return View(carton);
        }

        private CartonDetailsViewModel GetEquipmentList(int? id)
        {
            var carton = db.Cartons
               .Where(c => c.Id == id)
        ...
        public ActionResult AddEquipmentToCarton([Bind(Include = "CartonId,EquipmentId")] AddEquipmentViewModel addEquipmentViewModel)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    return RedirectToAction("AddEquipment", new { id = addEquipmentViewModel.CartonId, count = carton.CartonDetails.Count });
                }
                var equipment = db.Equipments
            ...
            return RedirectToAction("AddEquipment", new { id = addEquipmentViewModel.CartonId,count = 0 });
        }
        ...
        public ActionResult RemoveEquipmentOnCarton([Bind(Include = "CartonId,EquipmentId")] RemoveEquipmentViewModel removeEquipmentViewModel)
        {
           // return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            //if (ModelState.IsValid)
            {
                ...
                var dd = carton.CartonDetails.FirstOrDefault(a => a.EquipmentId == removeEquipmentViewModel.EquipmentId 
                                                                  && a.CartonId == removeEquipmentViewModel.CartonId);

                // db.Cartons.Find(carton.Id).CartonDetails.Remove(db.CartonDetails.Find(equipment.Id));
                db.CartonDetails.Remove(dd);
                db.SaveChanges();
            }
